(function () {
  ...
  function applyConfig(config) {
    ...
    setText("versionLabel", config.versionLabel);
  ...
  function setupFeedbackForm(config) {
    ...
    form.addEventListener("submit", async function (event) {
      ...
      if (!payload.deviceModel || !payload.systemVersion || !payload.appVersion || !payload.issueTime || !payload.issueType || !payload.actions) {
        setStatus(status, "请先填写所有必填项。", "error");
        return;
      }

      try {
        if (config.feedbackEndpoint) {
          await submitFeedback(config.feedbackEndpoint, payload);
          setStatus(status, "反馈已提交到线上接口，请继续保留截图和时间点。", "success");
        } else {
  ...
  /**
   * Submit a feedback payload to a configured HTTP endpoint.
   */
